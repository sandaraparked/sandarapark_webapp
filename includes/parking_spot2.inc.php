...
$sql='SELECT * FROM parking_spot2 LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
$result = mysqli_query($conn, $sql);
...
if(isset($_POST['update_parking_spot1'])) {
    ...
    $update = "UPDATE parking_spot2 SET area = ?, spot = ? , availability = ? WHERE parking_id= ? ";
    $stmt1 = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt1, $update)) {
      header("location: ../edit_parking_spot2.php?error=stmtfailed");
      exit();
    }
    ...
    $action_done = "edit the information of the parking spot  with parking id# $parking_id";
      ...
      $_SESSION['message'] = "Parking Space updated successfully"; 
	      header('location: ../reserve_exp2_admin.php');
        exit(0);
}
...
if(isset($_GET['del_parking']))
{
  ...
  if($del_run)
  {
    ...
    header('location: ../parking_spot2.php');
    exit(0);
  } 
